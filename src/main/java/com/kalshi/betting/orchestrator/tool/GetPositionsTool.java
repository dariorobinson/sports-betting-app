...
public class GetPositionsTool implements Supplier<String> {
    ...
    @Override
    public String get() {
        log.info("Calling Kalshi: get positions");
        try {
            String result = ToolServices.toJson(ToolServices.portfolioService.getPositions());
            log.info("Kalshi response (get positions): {}", result);
            return result;
        } catch (Exception e) {
            log.error("Kalshi call failed (get positions)", e);
